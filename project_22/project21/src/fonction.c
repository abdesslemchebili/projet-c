...
int verifx( char log[20],char pw [20])
{
FILE *f;
...
{
if ((strcmp(ch3,log)==0)&&(strcmp(ch4,pw)==0)){T=1;}
}
...
{	
	...
	{
	fprintf(F,"%s %s %s %s %s %s %s\n",u.id,u.nom,u.login,u.pw,u.type,u.sexe,u.role);
	fclose(F);
	return 1;
	}
...
{ F=fopen("utilisateur.txt","a+");
	...
	while (fscanf(F,"%s %s %s %s %s %s 	%s\n",id,nom,user_name,pw,type,sexe,role)!=EOF);
	fclose(F);
